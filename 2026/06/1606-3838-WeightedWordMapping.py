...
class Solution01:
    """
    Runtime 15ms Beats 15.97%
    Memory 19.34MB Beats 33.64%
    """

    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        a = ord("a")

        def get_char_weight(char: str) -> int:
            return weights[ord(char) - a]

        def get_total_weight(word: str) -> int:
            return sum(map(get_char_weight, word)) % 26

        def to_rev_char(weight: int) -> str:
            return chr(a + 25 - weight)

        def word_to_rev_char(word: str) -> str:
            return to_rev_char(get_total_weight(word))

        return "".join(map(word_to_rev_char, words))


class Solution02:
    """
    Runtime 7ms Beats 82.33%
    Memory 19.36MB Beats 33.64%
    """
    ...
    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        ...
        for i, word in enumerate(words):
            total_weight = sum([weights[ord(c) - a] for c in word]) % 26
            ans[i] = chr(a + 25 - total_weight)
...
class Solution03:
    """
    Runtime 8ms Beats 64.03%
    Memory 19.33MB Beats 33.64%
    """

    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        a, z = ord("a"), ord("z")
        return "".join(chr(z - sum(weights[ord(c) - a] for c in word) % 26) for word in words)
