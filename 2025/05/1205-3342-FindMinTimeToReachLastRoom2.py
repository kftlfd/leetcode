...
class Solution:
    """
    Runtime 1854ms Beats 60.94%
    Memory 90.65MB Beats 71.67%
    """
        ...
        while q:
            ...
            for nr, nc in ((r + dr, c + dc) for dr, dc in dirs):
                if not 0 <= nr < m or not 0 <= nc < n:
                    continue
                nt = max(t, moveTime[nr][nc]) + 1 + move
                if nt >= time[nr][nc]:
                    continue
                time[nr][nc] = nt
                heappush(q, (nt, nr, nc, move ^ 1))
...
class Solution1:
    ...
    Runtime 2762ms Beats 32.62%
    Memory 93.23MB Beats 55.36%
    """
