...
class Solution:
    """
    Runtime 4ms Beats 86.82%
    Memory 17.99MB Beats 81.99%
    """

    def validateCoupons(self, code: List[str], businessLine: List[str], isActive: List[bool]) -> List[str]:
        valid_blines = ["electronics", "grocery", "pharmacy", "restaurant"]

        ans = [
            (valid_blines.index(bline), ccode)
            for ccode, bline, active in zip(code, businessLine, isActive)
            if active and bline in valid_blines and re.fullmatch("\\w+", ccode)
        ]

        ans.sort()

        return [ccode for _, ccode in ans]
