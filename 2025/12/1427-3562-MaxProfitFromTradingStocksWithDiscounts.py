...
class Solution:
    ...
    def maxProfit(self, n: int, present: List[int], future: List[int], hierarchy: List[List[int]], budget: int) -> int:
        ...
        @cache
        def dfs(eid: int, discount: bool, budget: int, profit: int) -> tuple[int, int]:
            if not graph[eid] or budget <= 0:
                price = present[eid] // 2 if discount else present[eid]
                if price > budget:
                    return (profit, budget)
                return max(
                    (profit, budget),
                    (profit + future[eid] - price, budget - price)
                )

            bestProfit = (profit, budget)

            # don't buy
            for nxtOrder in permutations(graph[eid]):
                curProfit = profit
                curBudget = budget
                for nxtid in nxtOrder:
                    curProfit, curBudget = dfs(
                        nxtid, False, curBudget, curProfit)
                bestProfit = max(bestProfit, (curProfit, curBudget))
            ...
            # buy
            for nxtOrder in permutations(graph[eid]):
                curProfit = profit + future[eid] - price
                curBudget = budget - price
                for nxtid in nxtOrder:
                    curProfit, curBudget = dfs(
                        nxtid, True, curBudget, curProfit)
                bestProfit = max(bestProfit, (curProfit, curBudget))

            return bestProfit

        return dfs(0, False, budget, 0)[0]


class Solution1:
    """
    leetcode solution: Tree Dynamic Programming
    Runtime 9547ms Beats 43.63%
    Memory 18.78MB Beats 56.86%
    """
