...
class Solution:
    """
    Runtime: 45 ms, faster than 9.10% of Python3 online submissions for Integer to English Words.
    Memory Usage: 16.7 MB, less than 47.47% of Python3 online submissions for Integer to English Words.
    """
    ...
    def numberToWords(self, num: int) -> str:
        ...
        while num > 0:
            ...
            triplet = self.parse_triplet(cur, segm)
            ans = triplet + ans
            segm += 1

        return " ".join(ans)

    def parse_triplet(self, num: int, segm: int) -> List[str]:
        ...
        if hundreds > 0:
            s.append(self.num_repr[hundreds])
            s.append('Hundred')

        if tens <= 20:
            if tens > 0:
                s.append(self.num_repr[tens])
        else:
            tens, ones = divmod(tens, 10)
            s.append(self.num_repr[tens * 10])
            if ones > 0:
                s.append(self.num_repr[ones])

        if s and segm in self.segments:
            s.append(self.segments[segm])
...
class Solution1:
    ...
    Runtime: 35 ms, faster than 68.95% of Python3 online submissions for Integer to English Words.
    Memory Usage: 16.7 MB, less than 47.47% of Python3 online submissions for Integer to English Words.
    """

    # Arrays to store words for numbers less than 10, 20, and 100
    below_ten = ["", "One", "Two", "Three", "Four",
                 "Five", "Six", "Seven", "Eight", "Nine"]
    below_twenty = ["Ten", "Eleven", "Twelve", "Thirteen", "Fourteen",
                    "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"]
    below_hundred = ["", "Ten", "Twenty", "Thirty", "Forty",
...
class Solution2:
    """
    leetcode solution 2: Iterative
    Runtime: 31 ms, faster than 88.01% of Python3 online submissions for Integer to English Words.
    Memory Usage: 16.7 MB, less than 47.47% of Python3 online submissions for Integer to English Words.
    """
...
class Solution3:
    """
    leetcode solution 3: Pair-Based Approach
    Runtime: 36 ms, faster than 61.63% of Python3 online submissions for Integer to English Words.
    Memory Usage: 16.7 MB, less than 47.47% of Python3 online submissions for Integer to English Words.
    """
    ...
    def numberToWords(self, num: int) -> str:
        ...
        for value, word in self.number_to_words_map.items():
            ...
            if num >= value:
                ...
                suffix = "" if num % value == 0 else " " + \
                    self.numberToWords(num % value)
